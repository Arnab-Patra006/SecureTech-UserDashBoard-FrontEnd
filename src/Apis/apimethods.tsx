...
export const fetchUserDataById = async (id: number) => {
  const url = `http://localhost:9090/person/${id}`;
...
export const LogIn = async (values: MyLogin) => {
  await axios
    .post("http://localhost:9090/login", values)
    .then((res: any) => {
      localStorage.setItem("currId", res.data.myCurrentId);
      ...
      if (useStore.getState().currRole === "USER") {
        fetchUserDataById(res.data.myCurrentId);
      }
    })
    .catch((err) => {
      console.error(err);
...
export const postContact = async (values: MyContact) => {
  try {
    const response = await axios.post("http://localhost:9090/contact", values);
...
export const signUp = async (values: MySignUp) => {
  try {
    const response = await axios.post("http://localhost:9090", values);
